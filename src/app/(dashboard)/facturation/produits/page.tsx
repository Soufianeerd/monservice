...
export default function ProductsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const data = await productActions.findAllAction(user.organizationId);
      setProducts(data);
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await productActions.deleteAction(id, user.organizationId);
      await loadData();
    } catch (error) {
      console.error('Erreur', error);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end">
        <Link href="/facturation/produits/new" className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
          + Nouveau Produit
        </Link>
      </div>
            ...
            <div className="sm:hidden space-y-4">
              {filteredProducts.map((product) => (
                <Card key={product.id}>
                  ...
                  <CardFooter>
                    <Link href={`/facturation/produits/${product.id}/edit`} className="text-blue-600 hover:text-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded p-1" aria-label={`Modifier ${product.name}`}>
                  ...
                  {filteredProducts.map((product) => (
                    <TableRow key={product.id}>
                      ...
                      <TableCell className="text-right space-x-3">
                        <Link href={`/facturation/produits/${product.id}/edit`} className="text-indigo-600 hover:text-indigo-900 inline-flex focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded p-1" aria-label={`Modifier ${product.name}`}>
