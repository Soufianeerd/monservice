...
describe('Patient Portal, Billing Bridge & Communication Database Constraints (Session 14)', () => {
  ...
  beforeAll(async () => {
    ...
    // Setup Appointment
    await sql`INSERT INTO appointments (id, organization_id, practitioner_id, patient_id, appointment_type_id, location_id, created_by_user_id, starts_at, ends_at, occupancy_starts_at, occupancy_ends_at, timezone, status, created_at, updated_at) VALUES (${appointmentA}, ${orgA}, ${pracA}, ${patientA}, ${typeA}, ${locA}, ${userProA}, now() + interval '24 hours', now() + interval '25 hours', now() + interval '24 hours', now() + interval '25 hours', 'Europe/Paris', 'scheduled', now(), now()) ON CONFLICT DO NOTHING`;
  });
